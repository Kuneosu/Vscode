...
def insert() :
    ...
    print("1: 고객 / 2: 변호사 / 3: 법원 / 4: 사건 => ",end="")
    insert_input = input()
    ...
    # 법원 정보 입력
        ...
        print(f"전문분야가 {case_filed}인 변호사 내역입니다. 하나를 선택해주세요.")
        select_sql = (f"select 변호사번호 from 변호사 where 전문분야={case_filed}")
        ...
        create_case = (f"insert into 사건 (사건번호, 사건명, 상태, 사건등록일, 사건마감일, 사건종류, 위임인, 위임일자, 관할법원, 승패여부) values ('{case_number}','{case_name}','{case_progress}','{case_date}','{case_enddate}','{case_filed}','{case_customer}','{case_date}','{case_court}','{case_win}')")
        print(create_case)
        
        create_pa = (f"insert into 참여 (사건번호, 당사자번호) values ('{case_number}','{case_party}')")
        create_la = (f"insert into 변호 (사건번호, 변호사번호) values ('{case_number}','{case_lawyer}')")
main()